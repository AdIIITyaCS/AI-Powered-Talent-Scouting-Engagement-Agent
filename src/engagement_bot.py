...
class EngagementBot:
    ...
    def generate_interest_score(self, candidate: CandidateRecord, response_text: Optional[str] = None) -> float:
        if response_text is None:
            response_text = self.simulate_outreach(candidate, "")
        return self.evaluate_response(response_text)

    def score_candidate(self, candidate: CandidateRecord, response_text: Optional[str] = None) -> float:
        candidate.interest_score = self.generate_interest_score(
